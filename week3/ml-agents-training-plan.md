...
1. **파이썬 학습 환경** 구축 (2장)
2. **씬 오브젝트/태그/콜라이더** 정리 (3장)
3. **Agent 스크립트** 설계·작성 (4~5장)
4. **컴포넌트 부착·설정** (6장)
5. **보상 설계 확정** (7장)
6. **학습 설정(YAML)** 작성 (8장)
7. **학습 실행 → 모니터링 → 모델 적용** (9~11장)

각 단계는 앞 단계가 끝나야 다음이 의미가 있다. 특히 **3장(태그/콜라이더) 없이 4장(보상 판정)이 동작하지 않는다.**
...
### 2.1 왜 별도 가상환경인가
시스템 파이썬은 **3.13**인데, ML-Agents Release 4는 보통 **Python 3.10.x**를 요구한다(torch/mlagents 의존성). 그대로 쓰면 설치·실행에서 깨질 가능성이 크다. → **3.10 전용 venv를 따로 만든다.**
...
## 3. 씬 · 오브젝트 준비 (Unity 에디터 작업) — ✅ 완료

태그 `food`/`badFood`/`wall` 생성, Meat×8→`food`·Carrotfbx×5→`badFood`·wall×4→`wall` 할당 완료. 음식 콜라이더는 이미 `isTrigger`, Rogue는 Rigidbody+CapsuleCollider(+Player 태그) 확인, 씬 저장까지 끝. 트리거 방식으로 통일(보상 뒤 `Collectible.Respawn()` 재사용). 이후 4장부터 진행.

---

## 4. Agent 스크립트 설계 (구현할 내용 — 코드는 다음 단계)
...
> **프로젝트 제약 주의**: 참조 `FoodCollectorAgent.Heuristic()`은 `Input.GetKey`(레거시)를 쓴다. 이 프로젝트는 New Input System 전용이라 그대로 쓰면 예외가 난다. `PlayerController.Heuristic()`처럼 `Keyboard.current`(`wKey/sKey/aKey/dKey`)로 작성해야 한다.

### 4.3 행동 공간(Action Space) 결정
...
## 5. 관측(Observation) 설계 — "에이전트는 무엇을 보는가"

에이전트가 고기 방향으로 가고 당근을 피하려면 **주변 상황을 수치로** 받아야 한다. 두 축을 쓴다.

### 5.1 Ray Perception Sensor 3D (핵심)
...
- [ ] 태그 `food` / `badFood` / `wall` 생성
- [ ] Meat→`food`, Carrot→`badFood`, wall→`wall` 할당
- [ ] 음식 콜라이더 `isTrigger`, 에이전트 Rigidbody+Collider 확인
